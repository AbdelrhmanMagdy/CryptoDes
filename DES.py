class DES:
    ...
    def keyGen(self, key, round):
        ...
        return list(map(self.bin2hex, keys))
    ...
    def encryptOneTime(self, key, msg, decrypt):
        ...
        for i in range(16):
            msg = self.encryptOneRoud(keys[i], msg)
        msgSwap = msg[8:]+msg[:8]
        cipher = self.applyPerm(self.hex2bin(msgSwap), self.__fp)
        return self.bin2hex(cipher)

    def encrypt(self, key, msg, times, decrypt=False):
        cipher = msg
        for t in range(times):
            cipher = self.encryptOneTime(key, cipher, decrypt)
        return cipher